...
const { storeRepairs } = require("~/sqlite/queries/evidence");
...
const showRepair = async (id) => {
  try {
    ...
    const dataFormatted = await first("repairs", data, [
      "id",
  ...
  try {
    ...
    if (Object.keys(register.data).length === 0) {
      return {
        status: 500,
        message:
          "No se encntro la reparacion seleccionada, actualice la lista!",
      };
    }

    if (register.data.damage_id.length > 0) {
      const repairs_damage = register.data.damage_id;
      let deleteData = [];
      for (let i = 0; i < repairs_damage.length; i++) {
        deleteData[i] = await db.execSQL(
          "DELETE FROM repair_damage WHERE repair_id = ?",
          [repairs_damage[i].repair_id]
        );
      }
    }
...
const deleteRepairs = async (data) => {
  try {
    const db = await openDatabase();
    ...
    console.log("Hubo un error intentando eliminar los registros ", error);
  }
}


const updateRepaairs = async (data) => {
  try {
    await deleteRepairs(data);
    await storeRepairs({
      container_report_id: data.id,
      damages_repairs: data.repairs,
    });
  } catch (error) {
    console.log("Hubo un error intentando eliminar el registro ", error);
  }
